...
import pytest
from conftest import init_workspace, research_method, run_evolve
...
def _session(tmp_path: Path) -> Path:
    ...
    start_session(
        workspace,
        AgentLimits(10, 2, 2, max_cost_usd=10, max_wall_s=3600),
...
def _controller(path: Path, *, invalid_first: bool = False, cost: str = "0.25") -> Path:
    path.write_text(
        ...
        "import json, os\n"
        "from pathlib import Path\n"
        "assert os.environ['EVOLVE_CONTROLLER_ACTION_MODE'] == 'deferred'\n"
...
def test_controller_launcher_invalid_usage_blocks_further_calls(tmp_path: Path) -> None:
    workspace = _session(tmp_path)
    ...
    with pytest.raises(RuntimeError, match="unpriced_controller_cost"):
        launch_controller(workspace, config)
...
def test_controller_launcher_retains_unpriced_token_usage_without_a_dollar_limit(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="cost_usd is required"):
        launch_controller(workspace, config)
...
def test_controller_interruption_requires_explicit_resolution(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="was interrupted"):
        launch_controller(workspace, config)
    resolved = resolve_controller_interrupted(workspace, 1, "host restarted", total_tokens=12, cost_usd=0.1, wall_s=3)
    assert resolved["phase"] == "failed"
    assert resolved["usage"] == {"total_tokens": 12, "cost_usd": 0.1}
...
def test_controller_cli_runs_without_a_shell(tmp_path: Path) -> None:
    ...
    with script.open("a") as stream:
        stream.write(
            "from evolve.agent_driver import parse_action\nfrom evolve.agent_queue import defer_action\ndefer_action(Path(os.environ['EVOLVE_AGENT_WORKSPACE']), parse_action({'id':'finish','type':'finish_research','reason':'completed'}))\n"
        )
